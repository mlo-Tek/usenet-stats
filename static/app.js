let DATA=null,TAB="movies",METRIC="count",CUSTOM_RANGE=null,CURRENT=[],REFRESH_POLL=null;
...
const compactBytes=n=>{if(!n)return"0";const u=["B","K","M","G","T"];let i=0,v=Number(n);while(v>=1024&&i<u.length-1){v/=1024;i++}return`${v>=100?v.toFixed(0):v.toFixed(1)}${u[i]}`};
const fmtDate=d=>d?new Intl.DateTimeFormat("de-DE",{dateStyle:"medium",timeStyle:"short"}).format(new Date(d)):"–";
const fmtTime=d=>d?new Intl.DateTimeFormat("de-DE",{hour:"2-digit",minute:"2-digit"}).format(new Date(d)):"–";
const dayKey=d=>{d=new Date(d);return`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`};

/* ---------- Theme ---------- */
const THEME_KEY="usenet-stats-theme";
...
const DB_NAME="usenet-stats",DB_STORE="cache",DB_KEY="stats-v2";
...
async function idbGet(){try{const db=await openDb();return await new Promise((resolve,reject)=>{const tx=db.transaction(DB_STORE,"readonly"),r=tx.objectStore(DB_STORE).get(DB_KEY);r.onsuccess=()=>resolve(r.result||null);r.onerror=()=>reject(r.error)})}catch{return null}}
async function idbSet(value){try{const db=await openDb();await new Promise((resolve,reject)=>{const tx=db.transaction(DB_STORE,"readwrite");tx.objectStore(DB_STORE).put(value,DB_KEY);tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error)})}catch{}}
...
/* ---------- Sync / loading ---------- */
function renderSyncStatus(meta={},generatedAt=null,source="server"){
  const box=el("syncStatus"),text=el("syncText");
  box.classList.remove("refreshing","error");
  if(meta.lastError){box.classList.add("error");text.textContent=`Cache aktiv · Hintergrundfehler`;return}
  if(meta.refreshing){box.classList.add("refreshing");text.textContent="Aktualisierung im Hintergrund";return}
  if(source==="browser"){text.textContent=`Sofortansicht · Stand ${fmtTime(generatedAt)}`;return}
  if(generatedAt){text.textContent=`Aktuell · Stand ${fmtTime(generatedAt)}`;return}
  box.classList.add("refreshing");text.textContent="Daten werden geladen";
}

function hasRealPayload(j){return !!(j&&j.generatedAt&&Array.isArray(j.movies)&&Array.isArray(j.episodes))}
async function applyServerPayload(j){
  if(hasRealPayload(j)){
    DATA=j;
    await idbSet(j);
    populateFilters();
  ...
  renderSyncStatus(j?j._meta||{}:{},j?.generatedAt||DATA?.generatedAt||null,"server");
}
...
function scheduleRefreshPoll(){
  ...
  REFRESH_POLL=setTimeout(async()=>{
    try{
      const r=await fetch("/api/stats",{cache:"no-store"}),j=await r.json();
      if(r.ok)await applyServerPayload(j);
      if(j?._meta?.refreshing)scheduleRefreshPoll();
    }catch{scheduleRefreshPoll()}
  },2500);
}

async function load(force=false,{silent=false}={}){
  if(!silent)el("error").hidden=true;
  el("refresh").disabled=true;
  try{
    const r=await fetch(`/api/stats${force?"?refresh=1":""}`,{cache:"no-store"}),j=await r.json();
    if(!r.ok)throw new Error(j.error||`HTTP ${r.status}`);
    await applyServerPayload(j);
    if(j?._meta?.refreshing)scheduleRefreshPoll();
  }catch(e){
    if(!silent){el("error").textContent="Fehler: "+e.message;el("error").hidden=false}
    if(DATA)renderSyncStatus({lastError:e.message},DATA.generatedAt,"browser");
  }finally{el("refresh").disabled=false}
}

/* ---------- Filters ---------- */
function populateSelect(id,values,label){
  const s=el(id),old=s.value,vals=[...new Set(values.filter(Boolean))].sort((a,b)=>String(a).localeCompare(String(b),"de"));
...
function renderSummary(){
  ...
  el("movieSize").textContent=bytes(mb);el("episodeSize").textContent=bytes(eb);el("totalSize").textContent=bytes(mb+eb);el("generated").textContent=DATA?.generatedAt?"Stand "+fmtDate(DATA.generatedAt):"Wird geladen …";
...
function groupSeriesItems(items){
  ...
  return [...groups.values()].map(g=>{const newest=[...g].sort((a,b)=>b.timestamp-a.timestamp)[0],size=g.reduce((s,x)=>s+(x.size||0),0);return{...newest,grouped:true,episodeCount:g.length,size,sizeText:bytes(size),originalRelease:`${g.length} Episoden im Zeitraum`,releaseType:g.some(x=>x.releaseType==="season_pack")?"season_pack":"episode"}})
}
function renderTab(){
...
function renderIndexerPage(){
  ...
  const totalGrabs=stats.reduce((n,x)=>n+x.grabs,0),totalDownloads=stats.reduce((n,x)=>n+x.downloads,0);
  el("allGrabs").textContent=totalGrabs;el("allDownloads").textContent=totalDownloads;el("allSuccessRate").textContent=totalGrabs?`${(totalDownloads/totalGrabs*100).toFixed(1)} %`:"–";el("activeIndexerCount").textContent=stats.length;
...
function bindEvents(){
  ...
  el("compact").addEventListener("change",()=>{document.body.classList.toggle("compact",el("compact").checked);localStorage.setItem("usenet-stats-compact",el("compact").checked?"1":"0")});
  el("days").addEventListener("change",()=>{CUSTOM_RANGE=null;renderAll()});
  el("refresh").addEventListener("click",()=>load(true));
  document.querySelectorAll(".quick").forEach(b=>b.addEventListener("click",()=>setQuick(b.dataset.range)));
  el("applyDates").addEventListener("click",applyDates);el("clearFilters").addEventListener("click",clearFilters);
  el("preset4k").addEventListener("click",()=>{const q=[...el("quality").options].find(o=>/2160/i.test(o.value));if(q)el("quality").value=q.value;el("search").value="WEB";renderList()});
  el("exportCsv").addEventListener("click",()=>exportData("csv"));el("exportJson").addEventListener("click",()=>exportData("json"));el("closeDetails").addEventListener("click",()=>el("details").close());
  el("indexerSearch").addEventListener("input",renderIndexerPage);el("indexerSort").addEventListener("change",renderIndexerPage);
}

async function boot(){
  initTheme();bindEvents();
  const compact=localStorage.getItem("usenet-stats-compact")==="1";el("compact").checked=compact;document.body.classList.toggle("compact",compact);

  const cached=await idbGet();
  if(cached){DATA=cached;populateFilters();renderAll();renderSyncStatus(cached._meta||{},cached.generatedAt,"browser")}
  else renderSyncStatus({},null,"server");

  await load(false,{silent:!!cached});
  setInterval(()=>load(false,{silent:true}),60000);
}
